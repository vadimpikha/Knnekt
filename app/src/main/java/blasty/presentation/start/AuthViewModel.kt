...
import blasty.domain.usecase.ConnectycubeSignInUseCase
import blasty.domain.usecase.SendConfirmCodeUseCase
import blasty.domain.usecase.FirebaseSignInUseCase
...
class AuthViewModel(
    ...
    fun signIn() {
        ...
        viewModelScope.launch {
            firebaseSignInUseCase.execute(PhoneAuthData(verificationId, smsCode)).fold(::onError) { token ->
